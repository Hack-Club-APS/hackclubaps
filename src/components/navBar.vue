...
const isActive = computed(
    () =>
        props.alwaysShowGlass || pointerDown.value > 0.5 || glassVisible.value,
);
...
const $currentNav = useStore(currentNav);

watch(internalValue, (newVal) => {
    if ($currentNav.value !== newVal) {
        console.log("Syncing Store to:", newVal);
        currentNav.set(newVal);
    }
});
watch(
    $currentNav,
...
<template>
    ...
    >
        ...
        >
            ...
            <div
                class="absolute cursor-pointer transition-transform duration-100 ease-out z-40"
                :style="{
            ...
            >
                ...
                <div class="relative w-full h-full">
                    ...
                    <div
                        ...
                        :class="{
                            'bg-(--glass-rgb)/(--glass-bg-alpha) md:bg-[#FFFFFF]/80':
                                !isActive,
                        }"
                        :style="{
                    ...
                    :style="{
                        ...
                        transform:
                            internalValue === item.id
                                ...
                                : 'scale(1)',
                        mixBlendMode:
                            internalValue === item.id ? 'normal' : 'normal',
                    }"
                >
                    <div
                        v-if="item.icon"
                    ...
                    ></div>
                    <span
                        class="font-large text-black dark:text-white leading-none text-center truncate transition-colors"
                        :style="{
                            color:
                                internalValue === item.id
                                    ? `${props.color}`
